...
## MOVIMIENTO IA
def DrawMove(board,turnos,fila_mvm_IA,col_mvm_IA,combo_check):
    import random

    if(turnos==0): ## PRIMER MOVIMIENTO DE LA IA ALEATORIO
        entradaFilaIA = random.randrange(0,3,2)
        entradaColumnaIA = random.randrange(0,3,2)

        return entradaFilaIA,entradaColumnaIA

    elif(turnos == 2): ## SEGUNDO MOVIMIENTO IA
    ...
    elif (turnos == 4): ## SI EN EL TERCER TURNO, NO HAY VALOR EN EL CENTRO
        if(board[1][1]!= "X")and(board[1][1]!="O"):
            entradaFilaIA=1
            entradaColumnaIA=1
            return entradaFilaIA,entradaColumnaIA

    elif(win_condittion_var[0]==False): ## SI NO HAY WIN CONDITIION

        huecos = MakeListOfFreeFields(board) ## HUECOS DEL TABLERO DISPONIBLES
        for i in range(3):
            if(len(huecos[i])>=1): ## BUSCA LAS FILAS, DONDE HAYAN 1 O MAS VALORES DISPONIBLES
                entradaFilaIA = i
                entradaColumnaIA = huecos[i][0]
...
fila_mvm_IA=0 ## VALOR FILA IA (FUNCION DRAWMOVE)
col_mvm_IA=0  ## VALOR COLUMNA IA (FUNCION DRAWMOVE)
turnos = 0    ## TURNOS PARTIDA

while True:

    fila_mvm_IA2, col_mvm_IA2 = DrawMove(board,turnos,fila_mvm_IA,col_mvm_IA,combo_check) ## SE ASIGNAN VALORES DEL DRAWMOVE
    fila_mvm_IA=fila_mvm_IA2
    col_mvm_IA=col_mvm_IA2

    board[fila_mvm_IA][col_mvm_IA] = "X" ## MOVIMIENTO IA
    turnos += 1                          ## TURNO +1
    ...
    DisplayBoard(board)     ## PRINTEA EL MAPA

    MakeListOfFreeFields(board)     ## LISTA DE POSICIONES VACIAS

    EnterMove(board)    ## MOVIMIENTO USER
    turnos += 1         ## TURNO +1
...
## PRINTEA LA VICTORIA O EL EMPATE TECNICO
